...
Invoke-RestMethod -Method Post -Uri "http://localhost:8080/seed/reset-and-seed
" -ContentType "application/json" -Body '{"confirm":"RESET"}'

It must provide -Body '{"confirm":"RESET"}' as a safeguard to ensure storage does not accidentally get wiped. Running this will wipe the current data and create a fresh test dataset. It creates:

1 instructor user (used for teacher login)
2 classes (COSC4P02 and COSC4P91)
2 assignments per class
1 submission per assignment

After running the response will include the IDs and assignment keys that were generated which can be used when testing assignment verify / uploads.
